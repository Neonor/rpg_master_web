...
def cache_bg(img_path,bg_color,bd_color=None):
    ...
    """
    fileout = CACHE_FOLDER / "bg" / f"{path.splitext(path.split(img_path)[-1])[0]}_{bg_color}_{bd_color}.png"
    ...
    try:
        ...
        out = Image.new('RGBA', size_all,(0,0,0,0))
        out.paste(bg,(0,0),mask1)
        
        
        out.save(fileout, format='PNG')
        return fileout
    except:
        traceback.print_exc()
        return
